...
class KongguProcessor:
    ...
    def process_file_pairs(self):
        for filename in list(self.unprocessed_files): # 遍历未处理的文件列表
            ...
            if "_" in filename:
                ...
                # A：不会。所以配对逻辑是，只要找到就直接用对应的编号找出另一张图像的路径。然后使用
                if "temp" in filename:
                    temp_file = filename
    ...
    def cleanFolder(self):
        if os.path.exists(self.directory): # 确保文件夹存在
            for filename in os.listdir(self.directory): # 遍历文件夹
                file_path = os.path.join(self.directory, filename) # 拼接路径
